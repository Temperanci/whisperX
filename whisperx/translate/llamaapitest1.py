...
def translate2ch(file_path):
    ...
    # 构造输出文件路径
    # output_file_path = str((Path(fdir) / f'trans_{fname}').absolute())
    srt_file = str((Path(fdir) / fname).absolute()).replace('.txt', '.srt')
    output_srt_path = str((Path(fdir) / f'trans_{fname}').absolute()).replace('.txt', '.srt')
    ...
    for i, chunk in enumerate(text_chunks):
        ...
        # 将 JSON 数据写入到输出文件中
        print(resp.json())
        # append_to_file(resp.json().content, output_file_path)
